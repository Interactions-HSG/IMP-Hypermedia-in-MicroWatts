								...
								const otMessageInfo *p_message_info, otError aResult)
{
	...
	if (type != OT_COAP_TYPE_ACKNOWLEDGMENT &&
		...
		LOG_WRN("Received unexpected CoAP type: %d\n", type);
	}
	...
	uint8_t* context = (uint8_t*)p_context;

    LOG_INF("Received: %d", *context);
    // (void *p_context,  char *p_message,  char *p_uri, channelResponse result)
    uint8_t slice[length+1];
    memcpy(slice, received_data, length+1);
    Organization::onDirectResponse(p_context, slice, length, type, code);
}
...
otError CoapClient::sendRequest(const char* to, const char* uri,const int port, const char* query, const char* payload, otCoapType requestType, otCoapCode requestCode, uint32_t requestObserve,const void* pContext) {
    //ARG_UNUSED(item);
	otError 		error = OT_ERROR_NONE;
	otMessage 		*myMessage;
    ...
    bool           coapBlock     = true;
    otCoapBlockSzx coapBlockSize = OT_COAP_OPTION_BLOCK_SZX_64;
    BlockType      coapBlockType = (aCoapCode == OT_COAP_CODE_GET) ? kBlockType2 : kBlockType1;
	otCoapMessageSetBlockSzx(myMessage, coapBlockSize);
	#endif
	...
	do{
		...
		myMessageInfo.mPeerPort = port;
		otIp6AddressFromString(to, &myMessageInfo.mPeerAddr);
		if (error != OT_ERROR_NONE){ 
		...
		LOG_INF("Sending Message - Context %d", *(uint8_t*) pContext);
		void* ctx = const_cast<void*>(pContext); // Cast const void* to void*
		error = otCoapSendRequest(myInstance, myMessage, &myMessageInfo, coap_response_handler, ctx);		
	}
	while(false);

	if (error != OT_ERROR_NONE) {
		LOG_WRN("Failed to send request: %d\n", error);
